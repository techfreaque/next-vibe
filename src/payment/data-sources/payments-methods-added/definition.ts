/**
 * Payment Methods Added - Endpoint Definition
...
import { PAYMENTS_METHODS_ADDED_ALIAS } from "./constants";
import { scopedTranslation } from "./i18n";
const PaymentsMethodsAddedWidget = lazyWidget(() =>
  import("./widget").then((m) => ({ default: m.PaymentsMethodsAddedWidget })),
);

const { POST } = createEndpoint({
  scopedTranslation,
  aliases: [PAYMENTS_METHODS_ADDED_ALIAS],
  method: Methods.POST,
  ...
  fields: customWidgetObject({
    render: PaymentsMethodsAddedWidget,
    noFormElement: true,
