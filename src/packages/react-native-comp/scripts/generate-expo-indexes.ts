 ...
 * then creates corresponding index.tsx files in src/packages/react-native-comp/app/[locale]
...
import { ${wrapperType} } from "@/packages/react-native-comp/utils";
...
function generateIndexes(logger: EndpointLogger): GenerationResult {
  ...
  logger.debug("\n🔍 Scanning for Next.js pages and layouts...\n", "bright");
  ...
  for (const pageFile of pageFiles) {
    ...
    try {
      ...
      if (hasCustomIndexDirective(targetPath)) {
        logger.debug(
          `⏭️  Skipped (custom-index): ${relativePath}/index.tsx`,
          "yellow",
        );
      ...
      logger.debug(`✅ Generated: ${relativePath}/index.tsx`, "green");
  ...
  for (const layoutFile of layoutFiles) {
    ...
    try {
      ...
      if (hasCustomIndexDirective(targetPath)) {
        logger.debug(
          `⏭️  Skipped (custom-index): ${relativePath}/_layout.tsx`,
          "yellow",
        );
        result.skipped.push(relativePath);
        continue;
      ...
      logger.debug(`✅ Generated: ${relativePath}/_layout.tsx`, "green");
      result.created.push(relativePath);
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
...
function printSummary(result: GenerationResult, logger: EndpointLogger): void {
  logger.debug(`\n${"=".repeat(60)}`, "gray");
  logger.debug("📊 Generation Summary", "bright");
  logger.debug(`${"=".repeat(60)}\n`, "gray");

  logger.debug(`✅ Created:  ${result.created.length} files`, "green");
  logger.debug(
    `⏭️  Skipped:  ${result.skipped.length} files (custom-index)`,
    "yellow",
  );
  logger.debug(
...
function main(): void {
  ...
  logger.debug("\n🚀 Expo Router Index Generator", "bright");
  logger.debug(`${"=".repeat(60)}\n`, "gray");

  logger.debug(`📁 Source:     ${relative(PROJECT_ROOT, SOURCE_DIR)}`, "blue");
  logger.debug(
    `📁 Target:     ${relative(PROJECT_ROOT, TARGET_DIR)}\n`,
    "blue",
  );
