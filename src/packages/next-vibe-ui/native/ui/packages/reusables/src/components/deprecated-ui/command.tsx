...
const CommandContext = React.createContext<CommandContext<any>>(
...
interface WithForwardRefCommand extends React.FC<CommandWrapperProps<Data>> {
  <T extends Data>(
    props: CommandWrapperProps<T>,
  ): ReturnType<React.FC<CommandWrapperProps<T>>>;
}
...
interface WithForwardRefCommandList
  ...
  <T extends Data>(
    props: CommandSectionListProps<T>,
  ): ReturnType<
    React.FC<
...
>(({ className, textClass, children, ...props }, ref) => {
  return (
    <Pressable
      ref={ref}
      className={cn(
        "px-5 pt-6 bg-background border-t border-border",
        className,
      )}
      {...props}
    >
