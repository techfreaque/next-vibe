...
import { identityFragment, platformOverviewFragment, headlessContextFragment, subAgentGuardFragment, languageFragment, folderContextFragment, messageMetadataFragment, toolExecutionControlFragment, formattingFragment, extraInstructionsFragment, userNameFragment, bootstrapFragment, guestContextFragment, mediaCapabilitiesFragment, autonomyStatusFragment } from "@/app/api/[locale]/agent/ai-stream/system-prompt/system-prompt";
...
import { systemContextFragment, remoteInstancesFragment, sshConnectionsFragment } from "@/app/api/[locale]/remote-connection/system-prompt";
...
export async function loadAllPromptFragments(
  ...
  const [promptContextDataResult, codingAgentSettingDataResult, contactDataResult, cortexDataResult, desktopDataResult, favoritesDataResult, remoteInstancesDataResult, skillDataResult, webDataResult] = await Promise.allSettled([
  ...
  const codingAgentSettingData = codingAgentSettingDataResult.status === "fulfilled" ? codingAgentSettingDataResult.value : undefined;
  ...
  const autonomyStatusBuilt = promptContextData ? autonomyStatusFragment.build(promptContextData) : null;
  const bootstrapBuilt = promptContextData ? bootstrapFragment.build(promptContextData) : null;
  const codingAgentSettingBuilt = codingAgentSettingData ? codingAgentSettingFragment.build(codingAgentSettingData) : null;
  ...
  const extraInstructionsBuilt = promptContextData ? extraInstructionsFragment.build(promptContextData) : null;
  const favoritesBuilt = favoritesData ? favoritesFragment.build(favoritesData) : null;
  const folderContextBuilt = promptContextData ? folderContextFragment.build(promptContextData) : null;
  const formattingBuilt = promptContextData ? formattingFragment.build(promptContextData) : null;
  const guestContextBuilt = promptContextData ? guestContextFragment.build(promptContextData) : null;
  const headlessContextBuilt = promptContextData ? headlessContextFragment.build(promptContextData) : null;
  const identityBuilt = promptContextData ? identityFragment.build(promptContextData) : null;
  const languageBuilt = promptContextData ? languageFragment.build(promptContextData) : null;
  const mediaCapabilitiesBuilt = promptContextData ? mediaCapabilitiesFragment.build(promptContextData) : null;
  const messageMetadataBuilt = promptContextData ? messageMetadataFragment.build(promptContextData) : null;
  const platformOverviewBuilt = promptContextData ? platformOverviewFragment.build(promptContextData) : null;
  const remoteInstancesBuilt = remoteInstancesData ? remoteInstancesFragment.build(remoteInstancesData) : null;
  const skillBuilt = skillData ? skillFragment.build(skillData) : null;
  const sshConnectionsBuilt = remoteInstancesData ? sshConnectionsFragment.build(remoteInstancesData) : null;
  const subAgentGuardBuilt = promptContextData ? subAgentGuardFragment.build(promptContextData) : null;
  const systemContextBuilt = remoteInstancesData ? systemContextFragment.build(remoteInstancesData) : null;
  const toolExecutionControlBuilt = promptContextData ? toolExecutionControlFragment.build(promptContextData) : null;
  const userNameBuilt = promptContextData ? userNameFragment.build(promptContextData) : null;
  ...
  if (codingAgentSettingBuilt) { leading.push({ id: "coding-agent-setting", priority: codingAgentSettingFragment.priority, str: codingAgentSettingBuilt }); }
  ...
  if (sshConnectionsBuilt) { leading.push({ id: "ssh-connections", priority: sshConnectionsFragment.priority, str: sshConnectionsBuilt }); }
