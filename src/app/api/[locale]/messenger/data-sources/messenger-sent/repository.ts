...
export class QueryMessengerSentRepository {
  static async queryMessengerSent(data: {
  ...
  > {
    ...
    const rows = await db
      .select({
        bucket: resolutionBucketExpr(resolution, emails.sentAt).as("bucket"),
        cnt: count(),
      })
      .from(emails)
      .where(
        and(
          ...
          gte(emails.sentAt, range.from),
          lte(emails.sentAt, range.to),
        ),
      )
      .groupBy(sql`1`)
      .orderBy(sql`1`);
