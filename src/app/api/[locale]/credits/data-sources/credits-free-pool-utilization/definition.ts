/**
 * Credits Free Pool Utilization - Endpoint Definition
...
import { CREDITS_FREE_POOL_UTILIZATION_ALIAS } from "./constants";
import { scopedTranslation } from "./i18n";

import { lazyWidget } from "next-vibe-ui/unified/_shared/lazy-widget";
const CreditsFreePoolUtilizationWidget = lazyWidget(() =>
  import("./widget").then((m) => ({
    default: m.CreditsFreePoolUtilizationWidget,
  })),
);

const { POST } = createEndpoint({
  scopedTranslation,
  aliases: [CREDITS_FREE_POOL_UTILIZATION_ALIAS],
  method: Methods.POST,
  ...
  fields: customWidgetObject({
    render: CreditsFreePoolUtilizationWidget,
    noFormElement: true,
