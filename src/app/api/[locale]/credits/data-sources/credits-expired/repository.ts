/**
 * Credits Expired — Repository
 * Server-only. DB access.
 * Sum of EXPIRY transaction absolute amounts per resolution bucket.
 */
...
export class QueryCreditsExpiredRepository {
  static async queryCreditsExpired(data: {
  ...
  > {
    ...
    const rows = await db
      ...
      .where(
        and(
          eq(creditTransactions.type, CreditTransactionType.EXPIRY),
