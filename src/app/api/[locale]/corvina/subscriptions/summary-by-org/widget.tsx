...
import type { SubscriptionsSummaryByOrgResponseOutput } from "./definition";
import type { SubscriptionsSummaryByOrgT } from "./i18n";

type SummaryItem = SubscriptionsSummaryByOrgResponseOutput["items"][number];
  ...
  return (
    <Div className="flex items-center gap-3 py-1 text-xs">
      ...
      <Span className="text-muted-foreground">
        {resource.used}/{resource.quantity}
      </Span>
      {resource.expired && (
...
function SummaryRow({
  ...
  item: SummaryItem;
  compact: boolean;
  t: SubscriptionsSummaryByOrgT;
}): React.JSX.Element {
  const expiringSoon = isExpiringSoon(item.expirationDate);

  if (compact) {
    return (
      <Div className="py-1 border-b last:border-b-0 text-sm font-mono">
        <Div>
        ...
        {item.orgResourceId !== null && (
          <Div className="text-xs text-muted-foreground">
            {`  org:${item.orgResourceId}`}
          </Div>
        )}
...
export function SubscriptionsSummaryByOrgContainer(): React.JSX.Element {
    ...
    return (
      <Div className="font-mono text-sm p-2">
        ...
        {items.length === 0 ? (
          ...
          items.map((item, idx) => (
            <SummaryRow key={idx} item={item} compact={true} t={t} />
          ))
        )}
      </Div>
    );
    ...
    <Div className="flex flex-col min-h-0">
      ...
      <Div className="overflow-y-auto max-h-[min(700px,calc(100dvh-200px))] p-4">
        {isLoading ? (
          ...
          items.map((item, idx) => (
            <SummaryRow key={idx} item={item} compact={false} t={t} />
          ))
        )}
      </Div>
    </Div>
