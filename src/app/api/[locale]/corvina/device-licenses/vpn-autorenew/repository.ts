...
import type {
  DeviceLicenseVpnAutorenewRequestOutput,
  DeviceLicenseVpnAutorenewResponseOutput,
} from "./definition";

const CORVINA_DEVICE_LICENSES_VPN_AUTORENEW_PATH =
  "/api/v1/deviceLicenses/vpn/autorenew";
...
export class DeviceLicenseVpnAutorenewRepository {
  static async update(
    data: DeviceLicenseVpnAutorenewRequestOutput,
    logger: EndpointLogger,
    locale: CountryLanguage,
  ): Promise<ResponseType<DeviceLicenseVpnAutorenewResponseOutput>> {
    ...
    const result = await CorvinaClient.request<CorvinaDeviceLicenseOut>(
      {
        method: "PUT",
        path: CORVINA_DEVICE_LICENSES_VPN_AUTORENEW_PATH,
        body,
        service: "license",
      },
      logger,
    ...
    logger.info("[CORVINA] Device license VPN autorenew updated", {
      logicalId: data.logicalId,
    });
    return success({
