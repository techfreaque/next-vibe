...
function computeEmaInternal(points: TimeSeries, period: number): TimeSeries {
  if (points.length === 0) {
    return [];
  }
  const k = 2 / (period + 1);
...
export function computeMacd(
  ...
  const fastEma = computeEmaInternal(points, fastPeriod);
  const slowEma = computeEmaInternal(points, slowPeriod);
  ...
  const signalLine = computeEmaInternal(macdLine, signalPeriod);
