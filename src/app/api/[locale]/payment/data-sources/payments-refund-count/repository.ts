...
export class QueryPaymentsRefundCountRepository {
  static async queryPaymentsRefundCount(data: {
  ...
  > {
    ...
    const rows = await db
      .select({
        bucket: resolutionBucketExpr(resolution, paymentRefunds.createdAt).as(
          "bucket",
        ),
        cnt: count(),
      })
