...
export class QueryPaymentsDisputesCountRepository {
  static async queryPaymentsDisputesCount(data: {
  ...
  > {
    ...
    const rows = await db
      .select({
        bucket: resolutionBucketExpr(resolution, paymentDisputes.createdAt).as(
          "bucket",
        ),
        cnt: count(),
      })
      ...
      .where(
        and(
          gte(paymentDisputes.createdAt, range.from),
          lte(paymentDisputes.createdAt, range.to),
        ),
      )
      .groupBy(sql`1`)
      .orderBy(sql`1`);
