...
describe("GET /user/public/login/options", () => {
  ...
  it("returns general login options without email", async () => {
    ...
    expect(res.success, `GET login options failed: ${String(res.success === false && res.message)}`).toBe(true);
    ...
    expect(response.loginMethods.password.passwordDescription).toBeTypeOf("string");
  ...
  it("returns user-specific options when email is provided for known user", async () => {
    ...
    expect(res.success, `GET login options with email failed: ${String(res.success === false && res.message)}`).toBe(true);
    if (!res.success) return;
