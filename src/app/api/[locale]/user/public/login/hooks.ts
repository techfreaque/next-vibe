...
export function useLogin(
  ...
  >["onSuccess"] = useCallback(
    async (
      ...
      try {
        ...
        toast({
          title: t("app.api.user.public.login.success.title"),
          description: t("app.api.user.public.login.success.description"),
          variant: "default",
        });
        ...
        logger.debug("app.api.user.public.login.redirect", {
          redirectTo,
        });
        router.push(redirectTo);
      } catch (error) {
        logger.error(
          "app.api.user.public.login.process.failed",
          parseError(error),
        );
      }
    },
  ...
  >["onError"] = useCallback(
    async (
      ...
      logger.error("app.api.user.public.login.error", parseError(data.error));
      if (data.error?.message) {
        toast({
          title: t("app.api.user.public.login.errors.title"),
          description: t("app.api.user.public.login.errors.auth_error"),
          variant: "destructive",
        });
  ...
  const alert = useMemo((): FormAlertState | null => {
    ...
    if (isAccountLocked) {
      return {
        ...
        title: {
          message: "app.api.user.public.login.errors.accountLocked",
        },
        message: {
          message: "app.api.user.public.login.errors.accountLockedDescription",
        },
      };
    }
