/**
 * Users Email Verified - Repository
 * Server-only. DB access.
 * Snapshot: count of users with emailVerified=true per resolution bucket.
 */
...
export class QueryUsersEmailVerifiedRepository {
  static async queryUsersEmailVerified(data: {
  ...
  > {
    ...
    while (ts <= range.to.getTime()) {
      ...
      const [row] = await db
        .select({ cnt: count() })
        .from(users)
        .where(and(lt(users.createdAt, next), eq(users.emailVerified, true)));
