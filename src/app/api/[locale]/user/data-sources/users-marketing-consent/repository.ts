/**
 * Users Marketing Consent — Repository
 * Server-only. DB access.
 * Snapshot: count of users with marketingConsent=true per resolution bucket.
 */
...
export class QueryUsersMarketingConsentRepository {
  static async queryUsersMarketingConsent(data: {
  ...
  > {
    ...
    while (ts <= range.to.getTime()) {
      ...
      const [row] = await db
        ...
        .from(users)
        .where(
          and(lt(users.createdAt, next), eq(users.marketingConsent, true)),
        );
