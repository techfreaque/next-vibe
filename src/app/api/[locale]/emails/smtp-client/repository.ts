 ...
 * Comprehensive repository for SMTP operations including email sending, account management,
 * and service logic previously in service.ts
 */
  ...
  static async testConnection(
    ...
    try {
      ...
      const [account] = await db
        .select()
        .from(smtpAccounts)
        .where(eq(smtpAccounts.id, data.accountId))
        .limit(1);

      if (!account) {
        return fail({
          message: t("list.errors.notFound.title"),
          errorType: ErrorResponseTypes.NOT_FOUND,
          messageParams: {
            accountId: data.accountId,
          },
      ...
      return success({
        success: true,
        message: t("enums.testResult.success"),
      });
    } catch (error) {
