...
export class QueryNewsletterUnsubscribesRepository {
  static async queryNewsletterUnsubscribes(data: {
    ...
    const rows = await db
      .select({
        bucket: resolutionBucketExpr(
          resolution,
          newsletterSubscriptions.unsubscribedDate,
        ).as("bucket"),
        cnt: count(),
      })
      ...
      .where(
        and(
          ...
          gte(newsletterSubscriptions.unsubscribedDate, range.from),
          lte(newsletterSubscriptions.unsubscribedDate, range.to),
        ),
      )
      .groupBy(sql`1`)
      .orderBy(sql`1`);
