/**
 * Competitors API Route Handlers
...
export const { GET, POST, tools } = endpointsHandler({
  ...
  [Methods.GET]: {
    ...
    handler: async ({ user, logger }) => {
      return await competitorsRepository.getCompetitors(user.id, user, logger);
    },
  },
  [Methods.POST]: {
    email: undefined,
    handler: async ({ data, user, logger }) => {
      return await competitorsRepository.updateCompetitors(
        user.id,
        data,
        user,
        logger,
