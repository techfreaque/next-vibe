...
import type { DiscoveredRoute } from "@/app/api/[locale]/v1/core/system/unified-backend/cli/route-executor";
 ...
 */
interface HelpEndpointDefinition {
...
interface ZodWithDef {
  _def?: {
    ...
    values?: Record<string, string | number> | readonly (string | number)[];
    description?: string;
  };
...
function hasShape(
  schema: z.ZodTypeAny,
): schema is z.ZodTypeAny & { shape: Record<string, z.ZodTypeAny> } {
...
export class HelpHandler {
   ...
   */
  private generateParameterHelp(
    endpoint: HelpEndpointDefinition,
  ): ParameterHelp[] {
  ...
  private generateUsageString(
    route: DiscoveredRoute,
    endpoint: HelpEndpointDefinition | null,
  ): string {
  ...
  private async getEndpointDefinition(
    route: DiscoveredRoute,
  ): Promise<HelpEndpointDefinition | null> {
    interface HelpRouteModule {
      tools?: {
        definitions?: Record<string, HelpEndpointDefinition>;
      };
    }

    try {
      const routeModule = (await import(route.routePath)) as HelpRouteModule;
