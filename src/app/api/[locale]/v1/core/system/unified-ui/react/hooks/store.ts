...
import { generateStorageKey } from "@/app/api/[locale]/v1/core/system/unified-ui/react/storage-storage-client";
...
export const useApiStore = create<ApiStore>((set, get) => ({
  ...
  >(
    ...
    if (!options.disableLocalCache) {
      try {
        const cachedData =
          await getStorageItem<TEndpoint["TResponseOutput"]>(queryId);
        if (cachedData) {
          set((state) => {
  ...
  >(
    ...
    try {
      ...
      // Return error response with proper translation key
      return createErrorResponse(
        "app.api.v1.core.system.unifiedUi.react.store.errors.unexpected_failure",
        ErrorResponseTypes.INTERNAL_ERROR,
