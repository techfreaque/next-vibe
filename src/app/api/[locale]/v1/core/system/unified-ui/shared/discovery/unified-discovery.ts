...
import { getStaticEndpoints } from "@/app/api/[locale]/v1/core/system/unified-ui/ai-tool/endpoint-adapter";
  ...
  private applyFilters(
    ...
    if (options.includeMethods && options.includeMethods.length > 0) {
      filtered = filtered.filter((e) =>
        options.includeMethods?.includes(e.method),
      );
    }
    ...
    if (options.roles && options.roles.length > 0) {
      filtered = filtered.filter((e) =>
        options.roles?.some((role) => e.allowedRoles.includes(role)),
      );
    }
    ...
    if (options.requiresAuth !== undefined) {
      filtered = filtered.filter((e) => {
        const requiresAuth = !e.allowedRoles.includes("PUBLIC");
