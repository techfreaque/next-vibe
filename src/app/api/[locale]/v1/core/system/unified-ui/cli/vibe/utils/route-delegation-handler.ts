...
export class RouteDelegationHandler {
  ...
  private async getCliHandler(
    ...
    try {
      ...
      if (
        ...
        const handler =
          routeModule.tools.cli[
            route.method as keyof typeof routeModule.tools.cli
          ];
      ...
      // Legacy: Check for direct tools export with method
      if (
        routeModule.tools &&
        typeof routeModule.tools[route.method] === "function"
      ) {
        return routeModule.tools[route.method] as CliHandlerFunction;
      }

      return null;
    } catch (error) {
      logger.warn(`Failed to load CLI handler for ${route.routePath}:`, error);
      return null;
    }
  }
  ...
  > | null> {
    try {
      ...
      try {
        ...
        logger.debug("Error loading definition:", error);
  ...
  private async generateFormFromEndpoint(
    ...
    return await schemaUIHandler.generateForm(config);
  }
   ...
   */
  private sanitizeDataForRenderer(data: CliResponseData): Record<string, any> {
  ...
  private getMissingRequiredFields(
    ...
    try {
      ...
      logger.warn("Failed to validate schema:", error);
    }
