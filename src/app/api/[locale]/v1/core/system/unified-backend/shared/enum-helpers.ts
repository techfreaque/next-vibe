...
function createEnumObjectWithKeyValues<T extends Record<string, string>>(
  ...
  for (const key in enumMap) {
    // Map each key to itself as a string value (KEY -> "KEY")
    result[key] = key as keyof T as (typeof result)[typeof key];
  }
  Object.freeze(result);
  return result as { readonly [K in keyof T]: K };
}
...
>(
  ...
  // The options array uses the KEY as value and translation key as label
  // This allows the UI to display translated labels while storing short enum values in the database
  ...
  return {
    ...
    Value: "" as keyof T, // Use empty string so typeof gives string, which matches the union type
  };
}
...
export function createSimpleEnumOptions<T extends string>(
  ...
  const enumObj = {} as { readonly [K in T]: K };
  ...
  // Create Zod schema with string validation
  const schema = z.string().refine((value) => values.includes(value as T), {
...
export function enumToSelectOptions<T extends Record<string, string>>(
  ...
  return Object.entries(enumObject).map(([key, value]) => {
    ...
    const labelValue: string = labelPrefix ? `${labelPrefix}.${key}` : key;
    const label: string = labelValue;
    const optionItem = {
      value: enumValue,
      label: label,
