 ...
 */
export class AIToolsRepositoryImpl {
  /**
  ...
  async getTools(
    ...
    try {
      logger.debug("[AI Tools Repository] Calling getEndpointsAsync", {
        userContext: user,
        platform: Platform.AI,
      });
      filteredEndpoints = await ToolRegistry.getEndpointsAsync(
        user,
        Platform.AI,
        logger,
      );
    ...
    logger.info("[AI Tools Repository] Filtered endpoints by permissions", {
      totalEndpoints: ToolRegistry.getStats().totalEndpoints,
