...
/**
 * Normalize path for ID generation
 */
function normalizePathForId(apiPath: string): string {
...
function generateEndpointId(method: Methods, apiPath: string): string {
  const pathPart = normalizePathForId(apiPath);
...
function generateEndpointName(apiPath: string): string {
  return normalizePathForId(apiPath);
}
