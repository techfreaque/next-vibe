...
export class UnifiedGroupingService {
  ...
  private groupByPath(
    ...
    for (const endpoint of endpoints) {
      ...
      const pathSegments = endpoint.definition.path.slice(1, maxDepth + 1); // Skip "v1"
  ...
  private groupByMethod(
    ...
    for (const endpoint of endpoints) {
      const method = endpoint.definition.method;
  ...
  private groupByTag(
    ...
    for (const endpoint of endpoints) {
      const tags = Array.isArray(endpoint.definition.tags)
