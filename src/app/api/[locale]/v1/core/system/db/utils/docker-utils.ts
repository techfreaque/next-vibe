...
export function executeDockerCommand(
  ...
  return new Promise((resolve) => {
    ...
    if (description) {
      ...
      logger.debug(
        t("app.api.v1.core.system.db.utils.docker.executing_command", {
          command,
        }),
      );
    }
    ...
    const timeoutPromise = new Promise<void>((_resolve, reject) => {
      timeoutId = setTimeout(() => {
        ...
        reject(
          new Error(
            t("app.api.v1.core.system.db.utils.docker.command_timeout", {
              timeout,
              command,
            }),
          ),
    ...
    child.on("close", (code) => {
      ...
      if (!success && error) {
        logger.error(
          t("app.api.v1.core.system.db.utils.docker.command_failed", {
            code: code ?? 0,
            command,
          }),
        );
    ...
    Promise.race([
      ...
      logger.error(
        t("app.api.v1.core.system.db.utils.docker.command_error", {
          error: err.message,
        }),
      );
      resolve({
