...
```
POST /api/en/v1/core/browser/click
POST /api/en/v1/core/browser/navigate_page
POST /api/en/v1/core/browser/take_screenshot
... etc
```
...
- The old monolithic `/api/v1/core/browser` endpoint has been kept for backward compatibility but should be deprecated in favor of the individual tool endpoints
...
- `/src/app/api/[locale]/v1/core/browser/route.ts` - Legacy monolithic endpoint (deprecated)
- `/scripts/generate-browser-tools.ts` - TypeScript tool generator script
- `/src/app/api/[locale]/v1/core/browser/shared/tool-specs.ts` - TypeScript tool specifications (source of truth)
