  ...
  async setAuthToken(
    ...
    try {
      if (envClient.platform.isServer) {
        ...
        return fail({
          message:
            "app.api.v1.core.user.auth.authClient.errors.token_save_failed",
          errorType: ErrorResponseTypes.AUTH_ERROR,
        });
      }
  ...
  async getAuthToken(
    ...
    try {
      if (envClient.platform.isServer) {
        ...
        return fail({
          message:
            "app.api.v1.core.user.auth.authClient.errors.token_get_failed",
          errorType: ErrorResponseTypes.AUTH_ERROR,
        });
      }
      ...
      return fail({
        message: "app.api.v1.core.user.auth.authClient.errors.token_get_failed",
        errorType: ErrorResponseTypes.AUTH_ERROR,
        messageParams: { error: String(error) },
      });
    }
  ...
  async removeAuthToken(logger: EndpointLogger): Promise<ResponseType<void>> {
    try {
      if (envClient.platform.isServer) {
        ...
        return fail({
          message:
            "app.api.v1.core.user.auth.authClient.errors.token_remove_failed",
          errorType: ErrorResponseTypes.AUTH_ERROR,
        });
      }
