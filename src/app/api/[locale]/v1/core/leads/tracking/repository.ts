...
export class LeadTrackingRepository implements ILeadTrackingRepository {
    ...
    try {
      ...
      if (!result.success || !result.data) {
        return result.success
          ? fail({
              message: "app.api.v1.core.leads.tracking.errors.default",
  ...
  async createAnonymousLead(
    ...
    try {
      ...
      logger.error(
        "app.api.v1.core.leads.tracking.anonymous.error",
        parseError(error).message,
      );
      return fail({
        message: "app.api.v1.core.leads.tracking.errors.default",
      ...
      if (trackingLeadId !== currentLeadId) {
        try {
          ...
          if (isLoggedIn && user.id) {
            ...
            leadsLinked = true;
            logger.debug("app.api.v1.core.leads.tracking.click.linkedToUser", {
              trackingLeadId,
              userId: user.id,
              currentLeadId,
            });
