...
The campaign starter is organized into modular files and directories:

- **`campaign-starter.cron.ts`** - Main cron task definition and execution
- **`types.ts`** - TypeScript types and Zod schemas
- **`repository.ts`** - Repository implementation for database operations
- **`campaign-starter-config/`** - Environment-specific configurations
- **`distribution/`** - Lead distribution calculation logic
- **`i18n/`** - Translation files for different locales
