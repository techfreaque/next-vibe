...
export const { env: smsEnv } = defineEnv({
  ...
  ADMIN_NOTIFICATION_PHONE: { schema: z.string().optional(), example: "+1234567890" },

  // MessageBird provider
  MESSAGEBIRD_ACCESS_KEY: { schema: z.string().optional(), example: "your_messagebird_key", comment: "MessageBird provider" },

  // Twilio provider
  TWILIO_ACCOUNT_SID: { schema: z.string().optional(), example: "your_twilio_account_sid", comment: "Twilio provider" },
  TWILIO_AUTH_TOKEN: { schema: z.string().optional(), example: "your_twilio_auth_token" },
  TWILIO_REGION: { schema: z.string().optional(), example: "us1" },

  // AWS SNS provider
  AWS_ACCESS_KEY_ID: { schema: z.string().optional(), example: "your_aws_access_key", comment: "AWS SNS provider" },
  AWS_SECRET_ACCESS_KEY: { schema: z.string().optional(), example: "your_aws_secret_key" },
  AWS_REGION: {
    schema: z.enum([
  ...
  // HTTP provider
  SMS_HTTP_API_URL: { schema: z.string().optional(), example: "https://api.example.com/sms", comment: "HTTP SMS provider" },
  SMS_HTTP_API_KEY: { schema: z.string().optional(), example: "your_api_key" },
  SMS_HTTP_API_METHOD: { schema: z.string().optional(), example: "POST" },
  SMS_HTTP_PHONE_REGEX: { schema: z.string().optional(), example: "^\\+[1-9]\\d{1,14}$" },
  ...
  SMS_HTTP_CUSTOM_HEADERS: { schema: z.string().optional(), example: '{"X-Custom": "value"}' },
  SMS_HTTP_AUTH_SCHEME: { schema: z.string().optional(), example: "Bearer" },
  SMS_HTTP_CONTENT_TYPE: { schema: z.string().optional(), example: "application/json" },
});
