...
import { QueueRegistry, StreamRegistry } from "./core/stream";
...
export async function processNextQueuedMessage(
  ...
  const streamResult = await AiStreamRepository.createAiStream({
    ...
    t: aiStreamT,
    subAgentDepth,
    // Dequeue streams never carry the context explicitly — the THREAD anchor
    // (chat_threads.stream_context.fixtureContext) supplies it at setup.
