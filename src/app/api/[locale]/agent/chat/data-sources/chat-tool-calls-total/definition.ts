...
import { CHAT_TOOL_CALLS_TOTAL_ALIAS } from "./constants";
import { scopedTranslation } from "./i18n";

import { lazyWidget } from "@/app/api/[locale]/system/unified-interface/unified-ui/widgets/_shared/lazy-widget";
const ChatToolCallsTotalWidget = lazyWidget(() =>
  import("./widget").then((m) => ({ default: m.ChatToolCallsTotalWidget })),
);
...
const { POST } = createEndpoint({
  ...
  aliases: [CHAT_TOOL_CALLS_TOTAL_ALIAS],
  allowedRoles: [UserRole.ADMIN],

  fields: customWidgetObject({
    render: ChatToolCallsTotalWidget,
    noFormElement: true,
