/**
 * Chat Upvotes Total - Repository
 * Server-only. DB access.
 * Sum of all upvotes on chat messages per resolution bucket.
 */
...
export class QueryChatUpvotesTotalRepository {
  static async queryChatUpvotesTotal(data: {
  ...
  > {
    ...
    const rows = await db
      .select({
        ...
        total: sum(chatMessages.upvotes),
      })
      .from(chatMessages)
      .where(
        and(
