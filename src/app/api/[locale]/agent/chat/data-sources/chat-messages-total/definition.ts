...
import { CHAT_MESSAGES_TOTAL_ALIAS } from "./constants";
import { scopedTranslation } from "./i18n";

import { lazyWidget } from "@/app/api/[locale]/system/unified-interface/unified-ui/widgets/_shared/lazy-widget";
const ChatMessagesTotalWidget = lazyWidget(() =>
  import("./widget").then((m) => ({ default: m.ChatMessagesTotalWidget })),
);
...
const { POST } = createEndpoint({
  ...
  aliases: [CHAT_MESSAGES_TOTAL_ALIAS],
  allowedRoles: [UserRole.ADMIN],

  fields: customWidgetObject({
    render: ChatMessagesTotalWidget,
    noFormElement: true,
