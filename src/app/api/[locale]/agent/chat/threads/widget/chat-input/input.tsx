...
import { useInputHandlers } from "@/app/api/[locale]/agent/chat/hooks/use-input-handlers";
...
import { TOUR_DATA_ATTRS } from "@/app/api/[locale]/agent/chat/widget/welcome-tour/tour-config";
...
export function ChatInput({ className }: ChatInputProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    if (prevInputRef.current !== input) {
      prevInputRef.current = input;
      void saveDraftToStorage(draftKeyRef.current, input, loggerRef.current);
    }
  }, [input]);
  ...
  // isLoading (chatStore) clears after the POST returns, but the WS stream may
  // still be running.  Also check aiStream.isStreamingThread so the stop button
  // stays visible for the full duration of the stream.
  const isActivelyStreaming =
    isLoading ||
    (activeThreadId ? aiStream.isStreamingThread(activeThreadId) : false);
