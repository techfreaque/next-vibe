...
function createStaticReconciler(): ReturnType<typeof Reconciler> {
  ...
  const hostConfig = {
    ...
    // Static rendering must be fully synchronous. React uses scheduleTimeout to
    // defer passive-effect flushing when microtasks aren't supported. If we let
    // it schedule real timers, effects (e.g. Radix's requestAnimationFrame in a
    // useEffect) fire on a LATER tick - outside our try/catch - and any throw
    // there escapes the error boundary, causing React to re-render the root
    // forever (infinite log spam -> OOM). We no-op scheduling and instead drain
    // passive effects synchronously inside renderWithReconciler's guarded block.
...
function renderWithReconciler(
  ...
  // Circuit breaker. A component that throws during render (e.g. a web-only
  // Radix widget calling browser APIs on CLI) is caught by an error boundary,
  // whose componentDidCatch setState + fallback UI can itself throw, which the
  // parent boundary catches, and so on - React re-renders the root
  // synchronously on every recovery attempt, never returning from
  // flushSyncWork. Left unbounded this floods logs with millions of lines and
  // OOMs the process. We count reported errors and, past a threshold, throw a
  // sentinel out of the error callback to unwind the synchronous recovery loop
  // back into the try/catch below - a clean, bounded failure instead of a hang.
  const MAX_RENDER_ERRORS = 8;
  let errorCount = 0;
  let tripped = false;

  class RenderCircuitBreakerError extends Error {}

  const recordError = (label: string, err: Error): void => {
    errorCount += 1;
    // Only log the first handful; the rest are the same cascade repeating.
    if (errorCount <= 3) {
      logger.error(`[Fast Renderer] ${label}:`, parseError(err));
    } else if (errorCount === 4) {
      logger.error(
        `[Fast Renderer] Suppressing further render errors (widget render is looping; likely a web-only widget on the CLI platform).`,
      );
    }
    if (errorCount >= MAX_RENDER_ERRORS && !tripped) {
      tripped = true;
      // oxlint-disable-next-line restricted-syntax -- circuit breaker: must throw to abort the render loop and unwind the ink tree; there is no ResponseType channel here.
      throw new RenderCircuitBreakerError(
        `Fast renderer aborted after ${errorCount} render errors`,
      );
    }
  };
  ...
  const root = createContainer(
    ...
    "fast-renderer",
    (err: Error) => {
      recordError("Reconciler uncaught error", err);
    },
    (err: Error) => {
      recordError("Reconciler caught error (boundary)", err);
    },
    (err: Error) => {
  ...
  // The error cascade is driven mostly through console.error (React's DEV error
  // logging and WidgetErrorBoundary.componentDidCatch both call it), not our
  // reconciler callbacks. We temporarily replace console.error/warn with a
  // counting proxy: after MAX_RENDER_ERRORS it throws the sentinel, which
  // unwinds React's synchronous recovery loop back into the catch below. This
  // is what actually bounds the "infinite log spam -> OOM" failure.
  /* eslint-disable no-console */
  const originalConsoleError = console.error;
  const originalConsoleWarn = console.warn;
  type ConsoleArgs = Parameters<typeof console.error>;
  const guardConsole = (original: (...a: ConsoleArgs) => void) => {
    return (...a: ConsoleArgs): void => {
      recordError("Render error (console)", new Error(String(a[0] ?? "")));
      if (errorCount <= 3) {
        original(...a);
      }
    };
  };
  console.error = guardConsole(originalConsoleError);
  console.warn = guardConsole(originalConsoleWarn);
  ...
  try {
    ...
    // Drain passive effects synchronously RIGHT HERE, inside the try, so any
    // throw (e.g. a web-only widget calling requestAnimationFrame in useEffect)
    // is caught below instead of firing on a later scheduler tick where it
    // would escape every boundary and loop the root forever. scheduleTimeout is
    // a no-op in our host config, so this is the only place effects ever run.
    reconciler.flushPassiveEffects();
  } catch (err) {
    if (err instanceof RenderCircuitBreakerError) {
      logger.error(`[Fast Renderer] ${err.message} - aborting render.`);
    } else {
      logger.error("[Fast Renderer] Reconciler error:", parseError(err));
    }
    // Still attempt to return whatever tree we produced before it threw.
  } finally {
    // Restore console before doing anything else that might log.
    /* eslint-disable no-console */
    console.error = originalConsoleError;
    console.warn = originalConsoleWarn;
    /* eslint-enable no-console */

    // Tear the root down deterministically. Without this the container, its
    // fibers, and any queued work stay alive across renders and can be
    // re-scheduled, leaking memory over repeated CLI renders.
    try {
      reconciler.updateContainerSync(null, root, null, noop);
      reconciler.flushSyncWork();
    } catch {
      // Unmount best-effort; nothing actionable if teardown itself throws.
    }
  }
