...
import { scopedTranslation } from "next-vibe/logger/error-monitor/data-sources/error-logs-warnings/i18n";
import { lazyWidget } from "next-vibe/unified-ui/_shared/lazy-widget";
import { customWidgetObject } from "next-vibe/unified-ui/_shared/utils";

import { ERROR_LOGS_WARNINGS_ALIAS } from "./constants";
const ErrorLogsWarningsWidget = lazyWidget(() =>
  import("./widget").then((m) => ({ default: m.ErrorLogsWarningsWidget })),
);

const { POST } = createEndpoint({
  scopedTranslation,
  aliases: [ERROR_LOGS_WARNINGS_ALIAS],
  method: Methods.POST,
  path: [
    "system",
    "logger",
  ...
  fields: customWidgetObject({
    render: ErrorLogsWarningsWidget,
    noFormElement: true,
