...
const DEFAULT_MESSAGES: BoilerplateMessages = {
  ...
  i18nIndexWrongImport:
    "i18n/index.ts may only import `createScopedTranslation` from '@/i18n/core/scoped-translation' and `translations as enTranslations` from './en'.",
 ...
 * Allowed:
 *   - "server-only"
 *   - anything containing "unified-interface/shared/endpoints/route/multi" (endpointsHandler)
 *   - anything containing "unified-interface/shared/types/enums" (Methods)
 *   - "./definition"
 *   - "./repository"
 */
function isAllowedRouteImport(source: string): boolean {
  ...
  if (source.includes("unified-interface/shared/endpoints/route/multi")) {
    return true;
  }
  if (source.includes("/email")) {
    return true;
  }
  if (source.includes("unified-interface/shared/types/enums")) {
    return true;
  }
  return false;
}
    ...
    if (isIndex) {
      ...
      return {
        ImportDeclaration(node: OxlintASTNode): void {
          ...
          const isAllowed =
            source === "@/i18n/core/scoped-translation" || source === "./en";
