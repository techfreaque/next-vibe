...
import {
  ATLAS_PID_FILE,
  readPidFilePort,
} from "@/app/api/[locale]/system/server/pid";
...
>({
  ...
  try {
    ...
    const alias = endpoint.aliases?.[0];
    if (!alias) {
      return fail({
        message: t("testing.test.errors.notFound.title"),
        errorType: ErrorResponseTypes.NOT_FOUND,
        messageParams: {
          error: `Endpoint ${endpoint.path.join("/")} has no alias — required for browser test URL`,
        },
      });
    }
    ...
    const urlParams = urlPathParams
      ...
      : "";
    const widgetUrl = `http://localhost:${port}/en-US/tools/${alias}${urlParams ? `?${urlParams}` : ""}`;
    ...
    try {
      ...
      if (!testUser.isPublic) {
        ...
        const { SessionRepository } =
          await import("@/app/api/[locale]/user/private/session/repository");
      ...
      // ── 8. Screenshot (always) ────────────────────────────────────────────
      {
        ...
        const screenshotPath = join(
          process.cwd(),
          ".tmp",
          `browser-test-${alias}-${Date.now()}.png`,
        );
