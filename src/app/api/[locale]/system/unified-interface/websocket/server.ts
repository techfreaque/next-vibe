...
import { resolveSystemChannelUser } from "./channel";
...
function subscribeToChannel(
  ...
  // A remote connector subscribes to `system/{scope}/self`; resolve `self` to
  // the authenticated user's concrete id so the stored channel matches the one
  // the publisher emits on (publishers always use the concrete userId).
  const channel = resolveSubscriberChannel(ws, rawChannel);
...
/**
 * Resolve a subscriber's channel: `system/{scope}/self` → the authed user's
 * concrete channel. Public users key by leadId, private by id — matching
 * authorizeWsChannel's `userId` resolution.
 */
function resolveSubscriberChannel(
  ws: ServerWebSocket<WsConnectionData>,
  channel: string,
): string {
  const { user } = ws.data;
  const authedUserId = user.isPublic ? user.leadId : user.id;
  return resolveSystemChannelUser(channel, authedUserId);
}
