...
import type { ConnectionConfig } from "@/app/api/[locale]/system/unified-interface/websocket/remote-event-bridge/transport/connector";
import {
  acquireConnection,
  closeConnection,
  getWsConnection,
  ...
  openConnection,
} from "@/app/api/[locale]/system/unified-interface/websocket/remote-event-bridge/transport/connector";
  ...
  describe(`Connector E2E — WsConnection lifecycle (${_remoteUrl})`, () => {
    ...
    it(
      ...
      async () => {
        ...
        // Derive atlas local broadcast URL
        const appUrl =
          process.env["NEXT_PUBLIC_APP_URL"] ?? "http://localhost:3000";
        const localBroadcastPort = ((): number => {
          try {
            const mainPort = parseInt(new URL(appUrl).port || "3000", 10);
            return process.env["VIBE_DISABLE_PROXY"] === "true"
              ? mainPort + 1000
              : mainPort;
          } catch {
            return 3000;
        ...
        // Block documents on hermes's side via both DB and in-memory config:
        // 1. DB update persists the change for future reconnects/restartConnection calls
        // 2. Control message updates hermes's live WsConnection in-memory config immediately
        const pdbForScope = getProdDb();
        await pdbForScope.execute(
          sql`UPDATE remote_connections
              SET sync_scope = COALESCE(sync_scope, '{}'::jsonb) || '{"documents": false}'::jsonb
              WHERE user_id = ${prodUserId} AND instance_id = ${ATLAS_INSTANCE_ID}`,
        );
        await fetch(`http://127.0.0.1:${localBroadcastPort}/ws/broadcast`, {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            channel: `system/control/${prodUserId}`,
            event: "settings-update",
            data: { syncScope: { documents: false } },
          }),
        });
        // Give hermes time to process the control message before opening any connections
        await sleep(1000);
        ...
        // Restore hermes's atlas connection syncScope (DB + in-memory)
        await pdbForScope.execute(
          sql`UPDATE remote_connections
              SET sync_scope = COALESCE(sync_scope, '{}'::jsonb) || '{"documents": true}'::jsonb
              WHERE user_id = ${prodUserId} AND instance_id = ${ATLAS_INSTANCE_ID}`,
        );
        await fetch(`http://127.0.0.1:${localBroadcastPort}/ws/broadcast`, {
          method: "POST",
    ...
    it(
      ...
      async () => {
        ...
        // Derive atlas local broadcast URL (for updating hermes's in-memory WsConnection)
        const appUrl =
          process.env["NEXT_PUBLIC_APP_URL"] ?? "http://localhost:3000";
        const localBroadcastPort = ((): number => {
          try {
        ...
        // Block skills on hermes's side via both DB and in-memory config:
        // 1. DB update persists the change for future reconnects/restartConnection calls
        // 2. Control message updates hermes's live WsConnection in-memory config immediately
        const pdbForScope = getProdDb();
        await pdbForScope.execute(
          sql`UPDATE remote_connections
              SET sync_scope = COALESCE(sync_scope, '{}'::jsonb) || '{"skills": false}'::jsonb
              WHERE user_id = ${prodUserId} AND instance_id = ${ATLAS_INSTANCE_ID}`,
        );
        await fetch(`http://127.0.0.1:${localBroadcastPort}/ws/broadcast`, {
          method: "POST",
          ...
          body: JSON.stringify({
            channel: `system/control/${prodUserId}`,
            event: "settings-update",
            data: { syncScope: { skills: false } },
          }),
        });
        // Give hermes time to process the control message before opening connections
        await sleep(1000);
        ...
        await pdbForScope.execute(
          sql`UPDATE remote_connections
              SET sync_scope = COALESCE(sync_scope, '{}'::jsonb) || '{"skills": true}'::jsonb
              WHERE user_id = ${prodUserId} AND instance_id = ${ATLAS_INSTANCE_ID}`,
        );
        await fetch(`http://127.0.0.1:${localBroadcastPort}/ws/broadcast`, {
          method: "POST",
