...
program
  .command("build")
  .description(t("app.api.system.builder.cli.build.description"))
  .option(
    // eslint-disable-next-line i18next/no-literal-string
    "-c, --config <path>",
    t("app.api.system.builder.cli.build.configOption"),
    t("app.api.system.builder.cli.build.defaultConfig"),
  )
...
async function setPackageBuildConfig({
  ...
  if (!outPutFilesNameWithoutExtension) {
    // eslint-disable-next-line oxlint-plugin-restricted/restricted-syntax -- Build infrastructure needs to throw for configuration errors
    throw new Error(t("app.api.system.builder.errors.invalidOutputFileName"));
  }
  buildOptions.lib = {
    entry: inputFilePath,
    formats: ["es", "cjs"],
...
export async function getConfigContent(
  ...
  if (!isBuildConfigModule(importedModule)) {
    // eslint-disable-next-line oxlint-plugin-restricted/restricted-syntax -- Build infrastructure needs to throw for configuration errors
    throw new Error(t("app.api.system.builder.errors.invalidBuildConfig"));
  }
