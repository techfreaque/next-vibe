...
export function runSnykTest(
  ...
  try {
    ...
    return fail({
      message: "app.api.system.releaseTool.snyk.cliNotFound",
...
export function runSnykMonitor(
  ...
  if (!env["SNYK_TOKEN"]) {
    ...
    return fail({
      message: "app.api.system.releaseTool.snyk.tokenRequired",
  ...
  if (!env["SNYK_ORG_KEY"]) {
    ...
    return fail({
      message: "app.api.system.releaseTool.snyk.orgKeyRequired",
      errorType: ErrorResponseTypes.INVALID_FORMAT_ERROR,
      messageParams: { packageName },
    });
  }
