/**
 * Generate TanStack Routes Definition
 * API endpoint definition for generating TanStack Router route files from Next.js pages
 */
...
import { scopedTranslation } from "next-vibe/platforms/tanstack-start/i18n";
import { objectField, responseField } from "next-vibe/unified-ui/_shared/utils";
import { z } from "zod";

/**
 * Generate TanStack Routes Endpoint Definition
 */
const { POST } = createEndpoint({
  ...
  method: Methods.POST,
  path: ["system", "platforms", "tanstack-start", "generate"],
  examples: {
    responses: {
      default: {
        success: true,
        created: ["src/app-tanstack/routes/$locale/index.tsx"],
        skipped: [],
        errors: [],
        message: "TanStack routes generated successfully",
      },
    },
  },
