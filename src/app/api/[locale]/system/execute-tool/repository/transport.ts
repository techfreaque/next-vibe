 ...
 * required (Set-Cookie leadId, status handling). It lives in execute-tool
 * because talking to a peer instance is the executor's concern — dispatch
 * routing is ./routing.ts, connection-backed calls are
 * RouteExecuteRepository.runInProcessTyped, and this is the pre-connection
 * escape hatch underneath both.
 */
...
export class RemoteTransport {
  ...
  static async callRaw(params: RemoteCallParams): Promise<RemoteCallResult> {
    ...
    if (body !== undefined) {
      ...
      // deterministic. See ./remote.ts callToolDirect.
  ...
  static async callEndpointDirect<TDef extends CreateApiEndpointAny>(
    ...
    // The endpoint's route handler always returns a valid { success, data|... }
    // ResponseType body — including its own typed error responses. Pass it
    // through verbatim so callers get proper error types + translation keys.
    if (raw.body !== null) {
      // raw.body is a WidgetData JSON value that the route handler shaped as a
      // ResponseType. Launder through a plain object (same pattern the
      // execute-tool repo uses) to narrow to the endpoint's response type.
      const launder: ResponseType<TDef["types"]["ResponseOutput"]> = JSON.parse(
        JSON.stringify(raw.body),
      );
      return launder;
    }
    // No parsed body → transport-level failure (network error or non-JSON).
