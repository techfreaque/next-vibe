/**
 * Execute-tool ROUTING — where should a call (or a whole AI loop) run?
 *
 * Dispatch-target resolution is an execute-tool concern: the executor decides
 * whether work runs locally or on a peer, and over which connection. This
 * module owns that decision; the remote-connection domain keeps the connection
 * rows and negotiation (./transport.ts callRaw/callEndpointDirect cover the
 * pre-connection bootstrap cases only).
 *
