...
export function getFieldStyleClassName(
  ...
  if (isRequired && style === "highlight") {
    if (hasValue) {
      ...
      const colors = requiredSuccessVariants[completedColor];
      return {
        containerClassName: cn(
          baseContainerClassName,
    ...
    const colors = requiredNovalueVariants[requiredColor];
    return {
  ...
  if (optionalColor !== "none") {
    const colors = requiredNovalueVariants[optionalColor];
    return {
      containerClassName: cn(baseContainerClassName, colors.containerClassName),
      labelClassName: cn(baseLabelClassName, colors.labelClassName),
      inputClassName: cn(baseInputClassName, colors.inputClassName),
...
const requiredNovalueVariants = {
  transparent: {
    containerClassName: "p-4 rounded-lg border border-input",
