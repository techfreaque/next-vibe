...
export class QueryLeadsCreatedRepository {
  static async queryLeadsCreated(data: {
  ...
  > {
    ...
    const rows = await db
      .select({
        bucket: resolutionBucketExpr(resolution, leads.createdAt).as("bucket"),
        cnt: count(),
      })
      .from(leads)
      .where(
        and(gte(leads.createdAt, range.from), lte(leads.createdAt, range.to)),
      )
      .groupBy(sql`1`)
      .orderBy(sql`1`);
