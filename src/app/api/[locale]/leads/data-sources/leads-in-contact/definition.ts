/**
 * Leads In Contact - Endpoint Definition
...
import { LEADS_IN_CONTACT_ALIAS } from "./constants";
import { scopedTranslation } from "./i18n";

import { lazyWidget } from "next-vibe-ui/unified/_shared/lazy-widget";
const LeadsInContactWidget = lazyWidget(() =>
  import("./widget").then((m) => ({ default: m.LeadsInContactWidget })),
);
...
const { POST } = createEndpoint({
  ...
  aliases: [LEADS_IN_CONTACT_ALIAS],
  allowedRoles: [UserRole.ADMIN],

  fields: customWidgetObject({
    render: LeadsInContactWidget,
    noFormElement: true,
