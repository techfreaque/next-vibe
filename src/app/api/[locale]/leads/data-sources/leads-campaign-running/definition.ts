/**
 * Leads Campaign Running - Endpoint Definition
...
import { LEADS_CAMPAIGN_RUNNING_ALIAS } from "./constants";
import { scopedTranslation } from "./i18n";
const LeadsCampaignRunningWidget = lazyWidget(() =>
  import("./widget").then((m) => ({ default: m.LeadsCampaignRunningWidget })),
);
...
const { POST } = createEndpoint({
  ...
  aliases: [LEADS_CAMPAIGN_RUNNING_ALIAS],
  allowedRoles: [UserRole.ADMIN],

  fields: customWidgetObject({
    render: LeadsCampaignRunningWidget,
    noFormElement: true,
