...
export async function queryLeadsEmailClicks({
  timeRange,
  resolution,
}: {
  timeRange: TimeRange;
  ...
  const rows = await db
    .select({
      bucket: sql<string>`date_trunc(${trunc}, ${emailCampaigns.clickedAt})`.as(
        "bucket",
      ),
      cnt: count(),
    })
    ...
    .where(
      and(
        ...
        gte(emailCampaigns.clickedAt, timeRange.from),
        lte(emailCampaigns.clickedAt, timeRange.to),
      ),
    )
    .groupBy(sql`1`)
    .orderBy(sql`1`);
