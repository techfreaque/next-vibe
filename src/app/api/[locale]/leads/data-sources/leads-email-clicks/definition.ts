/**
 * Leads Email Clicks - Endpoint Definition
...
import { LEADS_EMAIL_CLICKS_ALIAS } from "./constants";
import { scopedTranslation } from "./i18n";

import { lazyWidget } from "next-vibe-ui/unified/_shared/lazy-widget";
const LeadsEmailClicksWidget = lazyWidget(() =>
  import("./widget").then((m) => ({ default: m.LeadsEmailClicksWidget })),
);
...
const { POST } = createEndpoint({
  ...
  aliases: [LEADS_EMAIL_CLICKS_ALIAS],
  allowedRoles: [UserRole.ADMIN],

  fields: customWidgetObject({
    render: LeadsEmailClicksWidget,
    noFormElement: true,
