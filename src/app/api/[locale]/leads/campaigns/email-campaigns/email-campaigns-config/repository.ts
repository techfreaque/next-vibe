...
import type { EmailCampaignsConfigT } from "./i18n";
import type {
...
export class EmailCampaignsConfigRepository {
  private static readonly EMAIL_CAMPAIGNS_TASK_ID = "email-campaigns";
  private static getDefaultConfig(): EmailCampaignsConfigGetResponseOutput {
    ...
    try {
      ...
      const [existing] = await db
        ...
        .where(
          eq(
            cronTasks.id,
            EmailCampaignsConfigRepository.EMAIL_CAMPAIGNS_TASK_ID,
          ),
        )
        .limit(1);

      if (!existing) {
        return success(EmailCampaignsConfigRepository.getDefaultConfig());
      }

      const defaults = EmailCampaignsConfigRepository.getDefaultConfig();
  ...
  static async updateConfig(
    ...
    try {
      ...
      const cronData: NewCronTask = {
        id: EmailCampaignsConfigRepository.EMAIL_CAMPAIGNS_TASK_ID,
        shortId: EmailCampaignsConfigRepository.EMAIL_CAMPAIGNS_TASK_ID,
        routeId: EmailCampaignsConfigRepository.EMAIL_CAMPAIGNS_TASK_ID,
