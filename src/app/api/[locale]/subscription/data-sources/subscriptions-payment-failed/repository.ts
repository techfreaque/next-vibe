...
export class QuerySubscriptionsPaymentFailedRepository {
  static async querySubscriptionsPaymentFailed(data: {
    ...
    const rows = await db
      ...
      .where(
        and(
          ...
          gte(subscriptions.paymentFailedAt, range.from),
          lte(subscriptions.paymentFailedAt, range.to),
        ),
      )
      .groupBy(sql`1`)
      .orderBy(sql`1`);
