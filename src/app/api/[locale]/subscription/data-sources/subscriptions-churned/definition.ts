/**
 * Subscriptions Churned - Endpoint Definition
...
import { SUBSCRIPTIONS_CHURNED_ALIAS } from "./constants";
import { scopedTranslation } from "./i18n";

import { lazyWidget } from "next-vibe-ui/unified/_shared/lazy-widget";
const SubscriptionsChurnedWidget = lazyWidget(() =>
  import("./widget").then((m) => ({ default: m.SubscriptionsChurnedWidget })),
);

const { POST } = createEndpoint({
  scopedTranslation,
  aliases: [SUBSCRIPTIONS_CHURNED_ALIAS],
  method: Methods.POST,
  ...
  fields: customWidgetObject({
    render: SubscriptionsChurnedWidget,
    noFormElement: true,
