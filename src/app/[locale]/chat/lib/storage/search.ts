import type { ChatState, ChatThread, ChatMessage } from "./types";
...
function calculateScore(
  ...
  query: string
): { score: number; matchedMessages: Array<{ message: ChatMessage; snippet: string }> } {
  const normalizedQuery = normalizeText(query);
  ...
  if (normalizedTitle.includes(normalizedQuery)) {
    ...
    // Partial title match
    const titleMatches = queryTerms.filter(term => normalizedTitle.includes(term)).length;
  ...
  if (thread.metadata?.preview) {
    ...
    if (normalizedPreview.includes(normalizedQuery)) {
      ...
      const previewMatches = queryTerms.filter(term => normalizedPreview.includes(term)).length;
  ...
  for (const message of messages) {
    ...
    if (normalizedContent.includes(normalizedQuery)) {
      ...
      const contentMatches = queryTerms.filter(term => normalizedContent.includes(term)).length;
      if (contentMatches > 0) {
  ...
  const daysSinceUpdate = (Date.now() - thread.updatedAt) / (1000 * 60 * 60 * 24);
 ...
 */
export function quickSearchThreads(state: ChatState, query: string): ChatThread[] {
