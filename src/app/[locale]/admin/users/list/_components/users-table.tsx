...
export function UsersTable({
  ...
  const formatDate = useCallback(
    (date: Date | string | number | null): string => {
      if (!date) {
        return t("users.admin.formatting.fallbacks.never");
      }
      const dateObj =
  ...
  const getStatusConfig = (
    ...
    if (!user.emailVerified) {
      return {
        ...
        label: "users.status.emailUnverified",
      };
    }
    return {
        ...
        <TableHeader>
          <TableRow>
            <TableHead className="min-w-[200px]">
              {t("users.admin.table.name")}
            </TableHead>
            <TableHead className="min-w-[200px]">
              {t("users.admin.table.email")}
            </TableHead>
