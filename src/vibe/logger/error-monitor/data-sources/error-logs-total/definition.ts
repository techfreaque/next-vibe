...
import { scopedTranslation } from "next-vibe/logger/error-monitor/data-sources/error-logs-total/i18n";
import { lazyWidget } from "next-vibe/unified-ui/_shared/lazy-widget";
import { customWidgetObject } from "next-vibe/unified-ui/_shared/utils";

import { ERROR_LOGS_TOTAL_ALIAS } from "./constants";
const ErrorLogsTotalWidget = lazyWidget(() =>
  import("./widget").then((m) => ({ default: m.ErrorLogsTotalWidget })),
);

const { POST } = createEndpoint({
  scopedTranslation,
  aliases: [ERROR_LOGS_TOTAL_ALIAS],
  method: Methods.POST,
  path: ["vibe", "logger", "error-monitor", "data-sources", "error-logs-total"],
  title: "post.title",
