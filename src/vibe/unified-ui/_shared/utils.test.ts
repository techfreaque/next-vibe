...
describe("extractSchemaDefaults", () => {
  ...
  describe("Edge cases", () => {
    ...
    it("handles primitive schemas without defaults", () => {
      ...
      expect(
        extractSchemaDefaults(z.coerce.number(), noopLogger),
      ).toBeUndefined();
    ...
    it("returns empty values for transformed string when forFormInit is true", () => {
      ...
      const result = extractSchemaDefaults<string>(
        schema,
        noopLogger,
        "",
        true,
...
describe("generateFormSchema", () => {
  it("merges request-data and url-path-param fields into one ZodObject", () => {
    ...
    // url-path-param field (top-level) is present in the form schema
    expect(keys).toContain("productId");
    // request-data fields are present too (this endpoint nests them under `fields`)
    expect(keys).toContain("fields");

    // The merged schema validates a flat object carrying BOTH buckets — the
    // url-path-param productId AND the nested request-data fields object.
    const parsed = formSchema.safeParse({
      productId: "123e4567-e89b-12d3-a456-426614174000",
      fields: { name: "Renamed product" },
    });
    expect(parsed.success).toBe(true);
  });

  it("matches the endpoint's stored formSchema", () => {
    // The definition factory builds formSchema via generateFormSchema, so the
    // stored schema must expose the same merged shape.
    ...
    expect(storedKeys).toContain("productId");
  });
});
