...
/**
 * Generate the combined form schema: request-data fields merged with
 * url-path-param fields into a single z.ZodObject. Forms validate and collect
 * BOTH sets — a pure url-path-param field (e.g. a `[rootFolderId]` SELECT) still
 * has a real form widget — then split the flat values into { data, urlPathParams }
 * on submit.
 *
 * The request-data schema is always a z.ZodObject (empty endpoints yield
 * `z.object({})`). The url-params schema is a z.ZodObject when the endpoint has
 * url-path-param fields, or `z.never()` when it has none — in which case the form
 * is just the request-data schema. Merging two z.ZodObjects yields a z.ZodObject,
...
export function generateFormSchema<F>(
  ...
  // normally an object (empty endpoints yield z.object({})); url-params is an
  // object only when the endpoint has url-path-param fields, else z.never().
  const requestShape =
    requestSchema instanceof z.ZodObject ? requestSchema.shape : {};
  const urlShape = urlSchema instanceof z.ZodObject ? urlSchema.shape : {};
  const formSchema = z.object({ ...requestShape, ...urlShape });
