...
import { customSkills } from "../../agent/skills/db";
...
async function patchHermesAtlasConnection(
  ...
  const { sendTestRequest } =
    await import("../../tooling/testing/testing-suite/send-test-request");
  const connByIdDef = (
    await import("../../remote-connection/[instanceId]/definition")
  ).default;
  ...
  describe(`Connector E2E — WsConnection lifecycle (${_remoteUrl})`, () => {
    ...
    it(
      ...
      async () => {
        ...
        const { cortexNodes } = await import("../../agent/cortex/db");
        const { CortexNodeType } = await import("../../agent/cortex/enum");
      ...
      async () => {
        ...
        // over the reverse-ws connection (updates atlas's row + REMOTE subfolder).
        {
          const { sendTestRequest } =
            await import("../../tooling/testing/testing-suite/send-test-request");
          const selfRenameDef = (
            await import("../../remote-connection/self/rename/definition")
          ).default;
        ...
        const { DefaultFolderId } = await import("../../core/execution-context");
