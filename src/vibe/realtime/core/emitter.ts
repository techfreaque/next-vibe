...
import { hasRemoteRelay, pushRemote } from "./relay-hook";
import type {
...
function getBroadcastUrl(): string {
  ...
  try {
    ...
    const wsPort = serverSystemEnv.VIBE_DISABLE_PROXY
      ? mainPort + 1000
      : mainPort;
...
function publishWsEvent<T extends AnyEndpointEventEnvelope>(
  ...
  // No proxy in this process AND none reachable over loopback — a standalone CLI
  // install has no WS server at all. Give up after the first connection failure
  // instead of paying a failed fetch (and logging a warning) on every emit.
  if (broadcastSinkUnreachable) {
    return;
  }
  ...
  // oxlint-disable-next-line restricted/restricted-syntax -- internal WS-proxy IPC (separate process)
  fetch(url, {
    method: "POST",
...
export function createEndpointEmitter<TEndpoint extends CreateApiEndpointAny>(
  ...
  return ((eventName: string, data?: EmitDataRuntime) => {
    ...
    // Passive in-process taps (e.g. the CLI live renderer) see EVERY event, and
    // see it before delivery is decided. Deliberately above the presence gate
    // and outside both delivery branches: an observer is not a WS subscriber, so
    // "no sockets on this channel" and "batched delivery" must not hide events
    // from it. Observing never changes where an event is actually delivered.
    notifyEventObservers(envelope);
    ...
    if (!user.isPublic && options?.fanOut !== false) {
      ...
      if (eventDecl?.remoteEvent === true) {
        ...
        // Registration hook, not an import: the bridge needs a database, which a
        // CLI/MCP install does not have. `relayed` reports whether a bridge was
        // actually there to take it, so a caller can still tell a dropped emit
        // from a fanned-out one.
        pushRemote(remoteEvent);
        relayed = hasRemoteRelay();
      }
    }
