...
Backtest runs are recorded in `pipeline_backtest_runs`, with per-datapoint output in `pipeline_backtest_results` (each tagged with `runId` referencing the run). The chart widget can overlay backtest results on the live chart.
