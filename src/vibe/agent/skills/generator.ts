...
import { GENERATED_DIR, getApiDir, getSrcDir, VIBE_DIR } from "@/env/paths";

const OUTPUT_FILE = `${GENERATED_DIR}/skills/index.ts`;

/**
 * Real locations of the modules the emitted files import. Both are resolved
 * from the GENERATED file's directory, not this one — hand-written "./config"
 * and "./embedding-type" were computed against this generator and pointed at
 * <generated>/skills/, which holds neither. The embedding files are emitted
 * beside each skill.ts, so their depth varies per skill and must be computed.
 */
const SKILL_CONFIG_MODULE = `${VIBE_DIR}/agent/skills/config.ts`;
const SKILL_EMBEDDING_TYPE_MODULE = `${VIBE_DIR}/agent/skills/embedding-type.ts`;
...
import type { Skill } from "${getRelativeImportPath(SKILL_CONFIG_MODULE, outputFile)}";
import type { SkillEmbedding } from "${getRelativeImportPath(SKILL_EMBEDDING_TYPE_MODULE, outputFile)}";
...
function buildSkillEmbeddingFile(
  ...
  return `import type { SkillEmbedding } from "${getRelativeImportPath(SKILL_EMBEDDING_TYPE_MODULE, outputFile)}";
...
async function generateSkillEmbeddings(
  ...
  const { computeEmbeddingHash, generateEmbedding } =
    await import("../cortex/embeddings/service");
  ...
  for (const filePath of skillFiles) {
    ...
    writeFileSync(
      embeddingFile,
      buildSkillEmbeddingFile(skillId, hash, embeddingStr, embeddingFile),
      "utf-8",
    );
...
export const generator: GeneratorDefinition = {
  ...
  /**
   * Only the skills index. The embedding files written alongside it are
   * API-gated (no embedding provider → none written), so listing them would
   * make the existence check fail permanently and the cache never hit.
   */
  output: OUTPUT_FILE,
  async generate(ctx) {
