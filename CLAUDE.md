...
**Thea** - Cloud AI on VPS. Monitors platform, delegates tasks to Hermes or Claude Code. MCP server: `thea`.
**Hermes** - Max's local AI, daily driver. Executes tasks, calls tools. MCP server: `hermes`.
**Claude Code (You)** - Coding agent running in Atlas. Execution agent for Thea, Hermes, or Max. MCP server: `atlas`.
...
- **CLAUDE.md is auto-generated.** Edit `src/app/api/[locale]/agent/chat/skills/default-skills/vibe-coder/skill.ts` → `vibe gen`
