ALTER TABLE "messenger_folders" ADD COLUMN "recent_count" integer DEFAULT 0;--> statement-breakpoint
...
-- Migrate imap_folders data into messenger_folders before dropping the table
...
-- Update emails.folder_id references from imap_folders rows to their messenger_folders counterparts
UPDATE "emails" e
SET "folder_id" = mf."id"
FROM "imap_folders" f
JOIN "messenger_folders" mf ON mf."account_id" = f."account_id" AND mf."path" = f."path"
WHERE e."folder_id" = f."id";--> statement-breakpoint

DROP TABLE "imap_folders" CASCADE;--> statement-breakpoint
