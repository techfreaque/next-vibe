...
// API endpoint
src/app/api/[locale]/user/i18n/en/index.ts → { errors: { notFound: "..." } }
Key: app.api.user.errors.notFound
...
❌ t("app.api.user.errors")            // Too generic, needs specific error name
```
...
```typescript
// src/app/api/[locale]/domain/i18n/en/index.ts
...
```typescript
// src/app/api/[locale]/domain/i18n/de/index.ts
...
Error: "app.api.user.errors.notFound" is not assignable to parameter of type 'TranslationKey'
...
grep -r "export const translations" src/app/api/[locale]/contact/i18n/en/index.ts
```
