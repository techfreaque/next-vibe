...
- **AGENTS.md is auto-generated.** Never edit it directly. Edit the source of truth instead: `src/app/api/[locale]/agent/chat/skills/default-skills/vibe-coder/skill.ts` (the `PROJECT_INSTRUCTIONS` constant). Then run `vibe gen` to regenerate AGENTS.md.
